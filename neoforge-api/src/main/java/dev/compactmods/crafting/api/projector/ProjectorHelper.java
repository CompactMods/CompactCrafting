...
public abstract class ProjectorHelper {
    ...
    public static Stream<PotentialMiniaturizationFieldLocation> findPotentialFields(LevelReader level, ProjectorPlacement starting) {
        final var s = Stream.<PotentialMiniaturizationFieldLocation>builder();
        for(var fieldSize : MiniaturizationFieldSize.values()) {
            final var fieldLocation = MiniaturizationFieldLocation.compute(fieldSize, starting);
